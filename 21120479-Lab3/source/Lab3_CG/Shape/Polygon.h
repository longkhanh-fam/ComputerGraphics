...
public:
	void rotateOutline(float rotationAngle)
	{
		rotationAngle = rotationAngle * M_PI / 180.0f; 
			double xC = (topLeft.getx() + bottomRight.getx()) / 2;
        double yC = (topLeft.gety() + bottomRight.gety()) / 2;
	...
	void rotateFilledPixel(float rotationAngle)
	{
		rotationAngle = rotationAngle * M_PI / 180.0f;  // Rotation angle in degrees
        ...
        matrix.rotate(rotationAngle);
       	matrix.translate(xC, yC);
		filledPixels = matrix.TransformPixels(filledPixels);
	}

	void translateOutline(double offsetX, double offsetY)
	{
		Matrix matrix;
		matrix.translate(offsetX, offsetY);
		vertices = matrix.TransformPoints(vertices);
	}
	void translateFilledPixel(double offsetX, double offsetY)
	{
		Matrix matrix;
		matrix.translate(offsetX, offsetY);
		filledPixels = matrix.TransformPixels(filledPixels);
	...
	//scale
	void scaleOutline(double scaleX, double scaleY)
	{
		Point center = calculateCenter(vertices);
		double xC = center.getx();
		double yC = center.gety();

		double newXC = scaleX * xC;
        double newYC = scaleY * yC;

		Matrix matrix;
		matrix.scale(scaleX, scaleY);
	   	matrix.translate(xC - newXC, yC - newYC);
		vertices = matrix.TransformPoints(vertices);
	}

	void scaleFilledPixel(double scaleX, double scaleY)
	{
		Point center = calculateCenter(vertices);
		double xC = center.getx();
		double yC = center.gety();
